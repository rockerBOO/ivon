...
def test_ivon_with_accelerate_gradient_accumulation(model_and_data):
    ...
    model, optimizer, dataloader = accelerator.prepare(model, base_optimizer, dataloader)
    ...
    initial_state = {
        ...
        'avg_grad': base_opt.state.get('avg_grad', None).clone() if base_opt.state.get('avg_grad') is not None else None,
        'avg_nxg': base_opt.state.get('avg_nxg', None).clone() if base_opt.state.get('avg_nxg') is not None else None,
        'avg_gsq': base_opt.state.get('avg_gsq', None).clone() if base_opt.state.get('avg_gsq') is not None else None,
    }
