...
def generate_regression_data(n_samples=200, input_dim=10, noise_std=0.1):
    ...
    hidden = torch.relu(
        torch.matmul(X, torch.tensor(true_weights1, dtype=torch.float32))
        + torch.tensor(true_bias1, dtype=torch.float32)
    )
    y = torch.matmul(
        hidden, torch.tensor(true_weights2, dtype=torch.float32)
    ) + torch.tensor(true_bias2, dtype=torch.float32)
...
def test_ivon_sampling_variance():
    ...
    model = RegressionNet()
    optimizer = IVON(model.parameters(), lr=0.01, ess=100, mc_samples=5)
...
def test_ivon_convergence_comparison():
    ...
    def train_model(optimizer_class, **optimizer_kwargs):
        ...
        if optimizer_class == IVON:
            ...
            optimizer = optimizer_class(
                model.parameters(), lr=0.001, weight_decay=1e-4, **optimizer_kwargs
            )
...
def test_ivon_uncertainty_estimation():
    ...
    model = RegressionNet()
    optimizer = IVON(model.parameters(), lr=0.01, ess=len(X), mc_samples=10)
...
def test_ivon_gradient_accumulation_robustness():
    ...
    model = RegressionNet()
    optimizer = IVON(model.parameters(), lr=0.01, ess=len(X), mc_samples=1)
...
def test_ivon_multi_device_support():
    ...
    # Test CPU training
    cpu_optimizer = IVON(cpu_model.parameters(), lr=0.01, ess=len(X_cpu), mc_samples=1)

    # Test CUDA training
    cuda_optimizer = IVON(
        cuda_model.parameters(), lr=0.01, ess=len(X_cuda), mc_samples=1
    )
